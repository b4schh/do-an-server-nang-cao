...
public async Task<IEnumerable<ComplexDto>> SearchByKeywordAsync(string keyword)
{
    keyword = keyword.Trim();

    var complexes = await _complexRepository.SearchByKeywordAsync(keyword);

    return complexes.Select(c => new ComplexDto
    {
    Name = c.Name,
    Street = c.Street,
    Ward = c.Ward,
    ...
    Description = c.Description
        ...
        public async Task<ComplexFullDetailsDto?> GetByNameAsync(string name)
    {
        if (string.IsNullOrWhiteSpace(name))
            return null;

        var complex = await _complexRepository
            .Query()
            .Include(c => c.Fields)              // sân con
            .Include(c => c.ComplexImages)              // hình ảnh          // tiện ích
            .AsNoTracking()
        ...
        return new ComplexFullDetailsDto
                {
            Name = complex.Name,
    ...
    public async Task<List<ComplexImageDto>> GetImagesAsync(int complexId)
{
    var images = await _complexImageRepository.GetByComplexIdAsync(complexId);

    if (images == null || images.Count == 0)
        return new List<ComplexImageDto>();

    return images.Select(img => new ComplexImageDto
    {
        Id = img.Id,
        ComplexId = img.ComplexId,
        ImageUrl = img.ImageUrl,
   ...
   public async Task<List<FieldPricing>> GetPricingAsync(
    int complexId,
    TimeSpan time)
{
    // 1. Lấy danh sách sân con trong complex
    var fields = await _fieldService.GetFieByComplexIdAsync(complexId);

    if (fields == null || !fields.Any())
        return new List<FieldPricing>();

    var result = new List<FieldPricing>();

    // 2. Với mỗi sân → tìm giá phù hợp
    foreach (var field in fields)
    {
        var pricing = await _timeSlotService.GetPricingAsync(
            field.Id,
            time
        );
...
public async Task<IEnumerable<ComplexDto>> SearchByDistrictAsync(string district)
{
    var complexes = await _complexRepository.SearchByDistrictAsync(district);
...
public async Task<List<ComplexDto>> GetSuggestionByDistrictAsync(
    string district,
