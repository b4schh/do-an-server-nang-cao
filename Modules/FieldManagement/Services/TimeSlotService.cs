...
namespace FootballField.API.Modules.FieldManagement.Services
{
    public class TimeSlotService : ITimeSlotService
    {
        ...
        public async Task<IEnumerable<TimeSlotPriceDto>> GetTimeSlotsByFieldIdAsync(int fieldId)
        {
            var timeSlots = await _timeSlotRepository.GetActiveTimeSlotsAsync(fieldId);
            return _mapper.Map<IEnumerable<TimeSlotPriceDto>>(timeSlots);
        }
        ...
        public async Task DeleteTimeSlotAsync(int id)
        {
          ...
          public async Task<TimeSlotPriceDto?> GetPricingAsync(
        int fieldId,
        TimeSpan time)
    {
        var slot = await _timeSlotRepository.GetByTimeAsync(fieldId, time);
        if (slot == null) return null;

        return new TimeSlotPriceDto
        {
